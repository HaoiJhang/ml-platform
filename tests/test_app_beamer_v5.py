...
def test_beamer_v5_zh_top_level_copy_is_localized(monkeypatch, tmp_path) -> None:
    ...
    assert "这个界面按 beamer 风格组织成引导式流程" in text_blob
...
def test_beamer_v5_english_switches_top_level_copy(monkeypatch, tmp_path) -> None:
    ...
    assert "This interface is organized as a beamer-style guided run" in text_blob
...
def test_beamer_v5_nav_dots_expose_section_jump_links(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("ML_PLATFORM_RUNS_DIR", str(tmp_path / "runs"))
    ...
    assert '?step=upload' in nav_markup
